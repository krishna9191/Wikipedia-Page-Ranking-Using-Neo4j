...
import org.apache.spark.SparkConf
	...
	def getKeywordAndFrameStatement(fname: String) {
		...
		val listOfFiles = sc.textFile("C:/pageRank/filelist.txt").toArray.foreach(line => getKeywordAndFrameStatement(line))
		
		printToFile(new File("C:/pageRank/nodeCreation.txt")){p=> nodeCreationArray.foreach(p.println)}
